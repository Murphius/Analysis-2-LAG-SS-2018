	...
	\begin{itemize}
		...
		\begin{align*}
			F(x) = & \sum_{k=0}^{\infty} \hat{f}(k) \exp\left( 2\pi i k 
				\frac{x}{p}\right) \\
			\text{wobei } \hat{f} (k) = & \int_0^1 \tilde{f}(x) \cdot 
				\exp(-2\pi i k x) \dd{x} \\
			 = & \int_0^1 f(p\cdot x)\exp(-2\pi i k x) \\
			\overset{t = px}{=} & \frac{1}{p}\int_0^p f(t) 
...
\begin{Definition}{
	...
	\begin{enumerate}
		\item \label{vl_19_stp_1}$d(x,y) = 0 \Leftrightarrow x = y$
		\item \label{vl_19_stp_2}$d(x,y) \leq d(x,z) + d(z,y)$ für alle $x,y,z \in X$
		\item \label{vl_19_stp_3}$d(x,y) = d(y,x)$ für $x,y \in X$
	\end{enumerate}
...
\begin{Bemerkung}{
	\begin{itemize}
		\item[ ]
		\item Stichpunkt~\ref{vl_19_stp_2} heißt Dreiecks-Ungleichung
		\item Stichpunkt~\ref{vl_19_stp_3} heißt Symmetrie
	\end{itemize}
}\end{Bemerkung}
...
\begin{Beispiel}{
	\begin{enumerate}
		\item[ ]
		\item Auf $\mathbb{K}$ definiert $d(x,y) = \abs{s -y}$ eine Metrik
		\item Auf $\mathbb{K}^n$ definieren wir die $l_1$-Metrik durch:
		\begin{align*}
			d_1( (x_1, \hdots, x_n), (y_1, \hdots, y_n) ) =
			 \sum_{i = 1}^n \abs{x_i - y_i}
		\end{align*}
		$d_1$ ist eine Metrik, denn \ref{vl_19_stp_1} und \ref{vl_19_stp_3} sind 
		 trivial. \ref{vl_19_stp_2} folgt mit: \\
		 ...
		 \end{align*}
		 \ref{vl_19_stp_1} und \ref{vl_19_stp_3} sind offensichtlich. Die 
	 ...
	 \item \label{vl_19_stp_4} Auf $\mathbb{K}^n$ definieren wir die $l_{\infty}$-Metrik:
		 \begin{align*}
		 	d_{\infty} (x,y) = \max\{\abs{x_i-y_i}\vert i = 1, \hdots, n\}
		 \end{align*}
		 \ref{vl_19_stp_1} und \ref{vl_19_stp_3} sind wieder einfach zu sehen. 
		 \ref{vl_19_stp_2} folgt aus: \\
		 Seien $x,y,z \in \mathbb{K}^n$ gegeben. Dann gilt:
		 \begin{align*}
		 	d_{\infty} (x,y) = & \max\{\abs{x_i-y_i}\vert i = 1, \hdots, n\} \\
		 	\leq & \max\{\abs{x_i - z_i} + \abs{z_i - y_i} \vert i = 1, \hdots, n\} \\
		 	\leq & \max\{\abs{x_i-z_i}\vert i = 1, \hdots, n\} + 
		 	\max\{\abs{z_i - y_i}\vert i = 1, \hdots, n\}  \\
...
\begin{Beispiel}{
	...
	\begin{enumerate}\setcounter{enumi}{4}
		...
		\end{align*}
		die sogenannte \emph{diskrete  Metrik} auf $X$. \ref{vl_19_stp_1} und 
		\ref{vl_19_stp_3} sind wieder einfach zu sehen. \\
		\ref{vl_19_stp_2}:  Ohne Einschränkung: Sei $x \neq y$ und $z \in X$ beliebig. Ist $z = x$ bzw. $z = y$, so gilt:
		\begin{align*}
		...
		\begin{align*}
			B(X) = \{f: X \rightarrow \mathbb{K}\vert f \text{ ist beschränkt}\}
		\end{align*}
		Wir definieren:
		\begin{align*}
		...
		\item $X$-beliebige Menge auf $B(X) =\{f: X \rightarrow \mathbb{K}\vert f 
		\text{ ist beschränkt}\}$ definieren wir:
		\begin{align*}
			d_\infty = \sup_{x \in X} \abs{f(x) -g(x)}
		\end{align*}
		\ref{vl_19_stp_1} und \ref{vl_19_stp_3} sind weiter einfach. 
		\ref{vl_19_stp_3} folgt aus: 
		\begin{align*}
			d_\infty (f,g) = & \sup_{x \in X} \abs{ f(x) -g(x)} 
...
\begin{Bemerkung}{
	\begin{itemize}
		...
		auf denen noch mehr Struktur vorliegt als \glqq nur\grqq{} eine Metrik.
	\end{itemize}
}\end{Bemerkung}