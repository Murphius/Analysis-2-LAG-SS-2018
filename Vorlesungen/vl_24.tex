...
\begin{proof}
	Sei $\epsilon >0$. Da $g$ stetig in $f(p)$ ist, gibt es $\delta\prime > 0$, 
	so dass
	\begin{align*}
		g(B_{\delta\prime}(f(p)) \subseteq B_\epsilon(g \circ f(p))
	\end{align*}
	Analog existiert $\delta > 0$ mit $f(B_\delta(p)) \subseteq B_{\delta\prime}
	(f(p))$. Dann gilt:
	\begin{align*}
		g \circ f (B_\delta(p)) \subseteq g(B_{\delta\prime}(f(p)) 
...
\begin{Einschub}{
	...
	\begin{align*}
		d_+ (Y,Y\prime) = d_+ ((X_1, \hdots, X_n), (X_1\prime, \hdots, X_n\prime)) 
		= \sum_{i = 1}^n d_i(x_i,x_i\prime)
	\end{align*}
	Eine weitere natürliche Metrik auf $Y$ ist gegeben durch:
	\begin{align*}
		d_{\max}(Y,Y\prime) = d_{\max}((X_1, \hdots, X_n),(X_1\prime, \hdots, 
			X_n\prime)) = \max_{i=1, \hdots, n} d_i(x_i,x_i\prime)
	\end{align*}
	Es zeigt sich, dass $d_+$ und $d_{max}$ äquivalent sind, denn:
	\begin{align*}
		d_\infty (Y, Y\times) \leq d_+(Y, Y\prime) \leq n \cdot d_{\max} 
		(Y, Y\prime)
	\end{align*}
...
\begin{Proposition}{%7 oder auch mal 9
	Seien $X_1, \hdots, X_n, Y$ metrische Räume und $f: Y \rightarrow X_1 \times 
	\hdots \times X_n$ mit $y \mapsto (f_1(Y), \hdots, f_n(Y))$, wobei 
...
\begin{proof}
	...
	Dann gilt:
	\begin{align*}
		\abs{d(x_1,x_2) + d(x_1\prime, x_2\prime)} = & 
		\abs{d(x_1,x_2) - d(x_1, x_2\prime) 
			+ d(x_1,x_2\prime) - d(x_1\prime,x_2\prime)} \\ \leq &
		\abs{d(x_1,x_2) - d(x_1, x_2\prime)} + 
			\abs{d(x_1,x_2\prime)-d(x_1\prime,x_2\prime)} \\ \leq &
		d(x_2, x_2\prime) + d(x_1, x_1\prime) = 
		d_+ ((x_1,x_2), (x_1\prime, x_2\prime))
	\end{align*}
	Ergo: Ist $d_+((x_1,x_2), (x_1\prime,x_2\prime)) < \delta$ (für $\delta = 
