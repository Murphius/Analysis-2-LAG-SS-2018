...
\begin{Bemerkung}{
	\begin{itemize}
		...
		\item Man vergleiche die obige Definition mit der Definition konvergenter 
		 ...
		 \end{align*} \todo{hier fehlt die Zahl vor dem Folgt-Pfeil}
	\end{itemize}
}\end{Bemerkung}
...
\begin{Definition}{\todo{gleichzeitig auch Proposition}%5 
...
\begin{Beispiel}{
	\begin{itemize}
		...
		$l_p$-Metrik
		\item Aus \todo{auf oder aus?} $V = \mathbb{K}^n$ definiert für $p \in \mathbb{N}$ 
		\begin{align*}
			\norm{\cdot}: V \rightarrow & [0, \infty)  \\
...
\begin{proof}
	...
	\begin{align*}
		d(y,z) \geq & d(x,y) - d(x,z) \\
			& d(x,z) - d(x,y)
	\end{align*} \todo{sieht so aus als ob in der 2. Zeile was fehlt?}
