...
\begin{Definition}{
	Sei $I \subset \mathbb{R}$ ein Intervall und $f : I \rightarrow \mathbb{R}$ 
	...
	$\alpha$. \\
	\textbf{Bemerkung:} Offensichtlich gilt: 
	$f(\alpha) = P_{n, \alpha} ( \alpha)$. Weiter gilt: 
	\begin{align*}
		f'(\alpha) = P_{n, \alpha}(\alpha) = \left( \sum_{l = 0}^n l 
...
\begin{Satz}[Satz von Taylor ( mit Lagrange-Restglied)]{
	...
	\end{align*}
	\textbf{Beweis:} Wähle $M \in \mathbb{R}$ mit 
	\begin{align*}
	...
	\end{align*}
	Daher ist zu zeigen: Ex existiert ein x zwischen $\alpha$ und $\beta$ mit:
	\begin{align*}
		f^{(n)}(x) = n! \cdot M
	\end{align*}
	...
	\begin{align*}
		...
		h(\alpha) = & f(\alpha) - P_{n-1, \alpha}(\alpha) M(\alpha -\alpha)^n = 0 
	...
	$h^{(n)}(x) = 0$. Damit gilt 
	\begin{align*}
		0 = h^{(n)}(x) = f^{(n)}(x) - P_{n-1, \alpha}(x) - M \cdot n!(x-\alpha)^0
	\end{align*}
	und daher $f^{(n)}(x) = M \cdot n!$ \\
	\textbf{Bemerkung:} die obige Darstellung des Restgliedes ist die sogenannte 
...
\begin{Korollar}{
	...
	\begin{itemize}
		...
		\item ist $n$ gerade, so ist $\alpha$ eine Extremstelle.
			...
			$\alpha$ Minimalstelle. \\
			\textbf{Beweis:} Wir betrachten nur den Fall $n$ gerade und 
			$f^{(n)}(\alpha) > 0$.\\
